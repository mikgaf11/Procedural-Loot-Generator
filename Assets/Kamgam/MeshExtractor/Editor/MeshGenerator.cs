...
namespace Kamgam.MeshExtractor
{
    public class MeshGenerator
    {
        ...
        /// </summary>
        /// <param name="referenceTransform">The transform used if 'transformToLastSelectedObject' is enabled.</param>
        /// <param name="transformToLocalTransform">Transform the mesh to local coordinates or keep them global? If combine meshes is turned and multiple objects are selected then the 'referenceTransform' will be used. This will be forced to FALSE if the mesh is a skinned mesh with bones or blend shapes.</param>
        ...
        public static List<Result> GenerateMesh(
            ...
            bool transformToLocalTransform,
            ...
            bool recordUndo = false)
        {
            ...
            // Prepare Mesh data (fill all the dictionaries from above).
            foreach (var tri in selectedTriangles)
            {
                ...
                // Do once for each mesh used by a selected triangle.
                if (!vertices.ContainsKey(sourceMesh))
                {
                    var component = tri.Component;

                    // Define targetTransform (remains null if transformToLocalTransform is disabled).
                    Transform localTargetTransform = null;
                    if (transformToLocalTransform)
                    {
                        if (combineMeshes)
                        {
                            // Use the reference transform if "combineMeshes" is turned on.
                            localTargetTransform = referenceTransform;
                        }
                        else
                        {
                            // Else use the seleted triange transform
                            localTargetTransform = tri.Component.transform;
                        }
                    }

                    bool isSkinned = extractBoneWeights || extractBlendShapes || tri.Component is SkinnedMeshRenderer;

                    // Create new triangle list for this mesh
                    selectedTrisPerMesh.Add(sourceMesh, new List<SelectedTriangle>());

                    // Calc pivot delta between the tris transform and the user defined pivot (in global space).
                    var pivotComponentGlobalPosition = tri.Transform.position;
                    Vector3 pivotDeltaGlobal = -pivotGlobalPosition; // Inverse direction because we push the vertices, not the pivot.
                    if (!isSkinned && localTargetTransform != null)
                    {
                        pivotDeltaGlobal = pivotComponentGlobalPosition - pivotGlobalPosition; // Inverse direction because we push the vertices, not the pivot.
                    }
                    bool hasPivotDelta = pivotDeltaGlobal != Vector3.zero;
                    if (!pivotDeltas.ContainsKey(tri.Component))
                    {
                        pivotDeltas.Add(tri.Component, pivotDeltaGlobal);
                    }

                    // Vertices
                    sourceMesh.GetVertices(getOrCreateMeshData(sourceMesh, vertices));
                    // Convert from local space to global space if not skinned.
                    // Skip if local > world > local transformation would change nothing.
                    if (!isSkinned && (localTargetTransform != tri.Transform || hasPivotDelta))
                    {
                        ...
                        for (int i = 0; i < numOfVertices; i++)
                        {
                            ...
                            if (hasPivotDelta)
                                vertices[sourceMesh][i] = vertices[sourceMesh][i] + pivotDeltaGlobal;

                            // Convert to selected local target.
                            if (localTargetTransform != null)
                            {
                                vertices[sourceMesh][i] = localTargetTransform.InverseTransformPoint(vertices[sourceMesh][i]);
                            }
                        }
                    }
                    ...
                    // Convert from local space to global space if not skinned.
                    // Skip if local > world > local transformation would change nothing.
                    if (!isSkinned && localTargetTransform != tri.Transform)
                    {
                        int numOfNormals = normals[sourceMesh].Count;
                        for (int i = 0; i < numOfNormals; i++)
                        {
                            // Convert to world space
                            normals[sourceMesh][i] = tri.Transform.TransformVector(normals[sourceMesh][i]);

                            // Convert to selected local target.
                            if (localTargetTransform != null)
                            {
                                normals[sourceMesh][i] = localTargetTransform.TransformVector(normals[sourceMesh][i]);
                            }
                        }
                    }
                    ...
                    {
                        // Convert from local space to global space if not skinned.
                        int numOfTangents = tangents[sourceMesh].Count;
                        for (int i = 0; i < numOfTangents; i++)
                        {
                            // Convert to world space
                            var t = tangents[sourceMesh][i];
                            var tVector = new Vector3(t.x, t.y, t.z);
                            tVector = tri.Transform.TransformVector(tVector);
                            t.x = tVector.x;
                            t.y = tVector.y;
                            t.z = tVector.z;
                            tangents[sourceMesh][i] = t;

                            // Convert to selected local target.
                            if (localTargetTransform != null)
                            {
                                t = tangents[sourceMesh][i];
                                tVector = new Vector3(t.x, t.y, t.z);
                                tVector = localTargetTransform.TransformVector(tVector);
                                t.x = tVector.x;
                                t.y = tVector.y;
                                t.z = tVector.z;
                                tangents[sourceMesh][i] = t;
            ...
            foreach (var kv in selectedTrisPerMesh)
            {
                ...
                for (int i = 0; i < newVertexIndices.Count; i++)
                {
                    ...
                    if (uvs[mesh].Count > 0) newUVs.Add(uvs[mesh][vertexIndex]);
