...
namespace Kamgam.MeshExtractor
{
    public static class ObjExporter
    {
        ...
        public static Dictionary<string, string> SaveMeshAsObj(
            string filePath, string objectName, Mesh mesh, IList<Material> materials,
            bool logFilePaths, bool recordUndo)
        {
            ...
            foreach (var kv in files)
            {
                ...
                // TODO: Evaluate if this would work instead of File.WriteAllText()
                //var asset = new TextAsset(content);
                //AssetDatabase.CreateAsset(asset, subdir + name);
                //if (recordUndo)
                //{
                //    Undo.RegisterCreatedObjectUndo(asset, "ME: OBJ file created");
                //}
