...
namespace Kamgam.MeshExtractor
{
    class UndoStack<T>
    {
        ...
        /// </summary>
        protected System.Action<T, bool> _assignNewStateFunc;
        ...
        /// The parameter will be one of the copies created by the 'copyStateFunc' and a boolean that is true for UNDO and false for REDO actions.
        /// You may want to copy the given value again if it is a reference type.</param>
        public UndoStack(System.Func<T, T> copyStateFunc, System.Action<T, bool> assignNewStateFunc, int maxEntries = 20)
        {
        ...
        public void Undo()
        {
            if (_undoStack.Count > 0) 
            {
                ...
                if (_undoStack.Count > 0)
                {
                    _assignNewStateFunc.Invoke(_undoStack.Last.Value, true);
                }
            }
        }
        ...
        public void Redo()
        {
            if (_redoStack.Count > 0)
            {
                ...
                _assignNewStateFunc(last, false);
            }
        }
    }
}
